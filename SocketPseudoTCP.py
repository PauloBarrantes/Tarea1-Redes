...
class SocketPseudoTCP(Node):
    ...
    def listen(self, nConnection):
        
        ''' Wait forever and return an item if one is immediately available,'''
        # queue = serverSockets[(sourceIp,sourcePort)].get()
        # queue.get(False)
    ...
    def connect(self, (destPort, destPort)):
